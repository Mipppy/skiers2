...
def mainPDFFunc():
  ...
  for seg in segments:
    ...
    if seg.find("Nordic") != -1 or seg.find("JNQ") != -1:
      ...
      for seg2 in minseg:
        if "underdogtiming.com/_files" in str(seg2['href']) and str(seg2).find(
            "Team") == -1 and str(seg2).find("Start") == -1:
    ...
    for race in sub:  #line in page
      if race != "":
        try:
          ...
          classKeywords = [
              "VARSITY BOYS", "VARSITY GIRLS", "JUNIOR VARSITY BOYS",
              "JUNIOR VARSITY GIRLS", "BEGINNER BOYS", "BEGINNER GIRLS",
          ...
          if parsed_race[0].isdigit() and parsed_race[1].isdigit(
          ) and time_pattern.match(parsed_race[2]) and score_pattern.match(
              parsed_race[3]) and parsed_race[4].isalpha():
...
def racerExistsAlready(race):
  ...
  for index, element in enumerate(race):
    if element and element[0].isupper() and all(
        c.islower() or not c.isalpha()
        for c in element[1:]) and not any(c.isdigit() for c in element):
      firstname = element
  ...
  if not racerQuery:
    new_racer = Racer(firstname=firstname,
                      lastname=lastname,
                      team=" ".join(elements_before_float))
    new_racer.save()
    return new_racer
  else:
    return racerQuery
...
def doesResultExistYet(race, racer):
  ...
  objects = Result.objects.filter(racer=racer[0],
                                  time=race[0][2],
                                  bib=int(race[0][0]),
                                  place=int(race[0][1]),
                                  score=truescore,
                                  ...
                                  level=race[2])
  if not objects:
    racer_result = Result(racer=racer[0],
                          time=race[0][2],
                          bib=int(race[0][0]),
                          place=int(race[0][1]),
                          score=truescore,
...
def additionalScoreShenaggings(race):
  ...
  score = scores[int(race[0][1]) -
                 1] if 1 <= int(race[0][1]) <= len(scores) else 0
  return score
...
def DirectRacerHit(q):
  try:
    ...
    racer = Racer.objects.filter(lastname__contains=lastname,
...
def compareScore(score, racer):
  ...
  original_racer_index = next(
      (i for i, d in enumerate(sortedMasterList) if d["racer"] == racer),
      None) + 1
